...
$app->group('/api', function (RouteCollectorProxy $group) {
    ...
    $group->get('/tasks', function (Request $request, Response $response) {
        ...
        $tasks = $controller->index($user->sub);
        $response->getBody()->write(json_encode($tasks));
        return $response->withStatus(200)->withHeader('Content-Type', 'application/json');
    });

    $group->get('/tasks/{id}', function (Request $request, Response $response, array $args) {
        $database = new db();
        $controller = new TaskController($database);
        $user = $request->getAttribute('user');
        $id = (int)$args['id'];
        $task = $controller->getTaskById($user->sub, $id);
        if ($task) {
    ...
    $group->put('/tasks/{id}/status', function (Request $request, Response $response, array $args) {
        $database = new db();
        $controller = new TaskController($database);
        $user = $request->getAttribute('user');
        $id = (int)$args['id'];
        ...
        $result = $controller->update($user->sub, $id, ['status' => $status]);
        $response->getBody()->write(json_encode($result));
        return $response->withStatus(200)->withHeader('Content-Type', 'application/json');
    });
